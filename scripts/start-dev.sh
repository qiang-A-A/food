...
# ---- 4. 启动前台 (5173) 与 后台 (5174) Vite 开发服务 ----
echo "▶ 启动前台 (5173)…"
cd "$ROOT"
"$ROOT/node_modules/.bin/vite" --config "$ROOT/frontend/vite.config.ts" \
  > /tmp/tsgq-frontend.log 2>&1 &
FRONTEND_PID=$!

echo "▶ 启动后台 (5174)…"
"$ROOT/node_modules/.bin/vite" --config "$ROOT/admin/vite.config.ts" \
  > /tmp/tsgq-admin.log 2>&1 &
ADMIN_PID=$!
...
echo "   停止全部服务：Ctrl+C 或 分别结束 PIDs $BACKEND_PID $FRONTEND_PID $ADMIN_PID"
